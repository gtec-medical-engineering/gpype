...
class _HDF5ReaderCore(RecordingReader):
    """Internal node replaying a plain HDF5 (.h5) recording."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            file_name: Path to the .h5 recording.
            ...
            variable_name: Name of the dataset
                :class:`~gpype.HDF5Writer` wrote the recording under.
            has_time_row: Whether the dataset's first row is a time axis
                rather than a signal channel.
...
class HDF5Reader(ioc.OChain):
    """Replays a plain HDF5 (.h5) recording as if it were a live source.

    Reads back what :class:`~gpype.HDF5Writer` writes -- sampling rate,
    channel labels and roles from the ``gpype_meta`` document, falling
    back to native HDF5 attributes and then to the recorded time column
    when metadata is absent, and to the caller's own ``sampling_rate``
    when none of those can supply one -- which is what lets this read a
    bare third-party HDF5 dataset as long as the rate is given.
    """
    ...
    def create_internal_nodes(self) -> List[ioc.Node]:
        ...
        nodes.extend(
            raw.source_stage(
                self, lambda: _HDF5ReaderCore(**self._core_params)
            )
        )
