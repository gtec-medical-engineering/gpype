"""Stream 8 synthetic channels plus keyboard markers over UDP.

Headless on purpose: a UDP sender needs no GUI. Router merges the signal
and the event stream, and arrow keys become event codes. Receive it with
example_basic_udp_receive.py. UDP drops packets rather than delaying
them, which is the trade this example is making.

Requires: the devices extra (Keyboard)
Run: python example_basic_udp_send.py
"""
import gpype as gp
